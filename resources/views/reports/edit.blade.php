...
@section('title', 'Editar Informe - ' . $report->title)

@section('content')
<h2 class="text-2xl font-bold mb-4">Editar Informe: <span class="font-normal">{{ $report->title }}</span></h2>
        ...
        <div class="flex flex-col md:flex-row gap-5">
            <div class="w-full md:w-1/2">
                ...
                <input type="date" name="report_date" id="report_date"
                    ...
                    value="{{ old('report_date', $report->report_date ? (\Carbon\Carbon::parse($report->report_date)->format('Y-m-d')) : '') }}" required>
                @error('report_date')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div class="w-full md:w-1/2">
                ...
                <input type="time" name="report_time" id="report_time"
                    ...
                    value="{{ old('report_time', $report->report_time ? (\Carbon\Carbon::parse($report->report_time)->format('H:i')) : '') }}" required>
                @error('report_time')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
                ...
                <div>
                    ...
                    <select id="cameras" class="w-full border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white" multiple size="5">
                        @foreach ($cameras as $camera)
                            {{-- El JS se encargará de deshabilitar las ya seleccionadas --}}
                ...
                <div>
                    ...
                    <select name="police_station_id" id="police_station_id" class="w-full border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white @error('police_station_id') border-red-500 @enderror" required>
                        ...
                        @foreach ($policeStations as $policeStation)
                            <option value="{{ $policeStation->id }}" {{ old('police_station_id', $report->police_station_id) == $policeStation->id ? 'selected' : '' }}>{{ $policeStation->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="cause_id" id="cause_id" class="w-full border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white @error('cause_id') border-red-500 @enderror" required>
                        ...
                        @foreach ($causes as $cause)
                            <option value="{{ $cause->id }}" {{ old('cause_id', $report->cause_id) == $cause->id ? 'selected' : '' }}>{{ $cause->cause_name }}</option>
                        @endforeach
                    </select>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function initializeMultipleSelectHandler(
        ...
        if (Array.isArray(oldValues) && oldValues.length > 0) {
            Array.from(selectElement.options).forEach(option => {
                if (oldValues.includes(option.value) || oldValues.includes(parseInt(option.value))) { // Check for string and int comparison
    ...
    // Para el formulario de EDICIÓN, pasamos los valores actuales del reporte como el segundo argumento de old()
    // si no hay datos 'old' de una validación fallida.
    initializeMultipleSelectHandler( "cameraSearch", "cameras", "addCamera", "selectedCamerasList", "hiddenCamerasContainer", "cameras[]", "cámara", {!! json_encode(old('cameras', $report->cameras->pluck('id')->toArray())) !!} );
    initializeMultipleSelectHandler( "victimSearch", "victims", "addVictim", "selectedVictimsList", "hiddenVictimsContainer", "victims[]", "víctima", {!! json_encode(old('victims', $report->victims->pluck('id')->toArray())) !!} );
    initializeMultipleSelectHandler( "accusedSearch", "accuseds", "addAccused", "selectedAccusedsList", "hiddenAccusedsContainer", "accuseds[]", "sospechoso", {!! json_encode(old('accuseds', $report->accuseds->pluck('id')->toArray())) !!} );
    initializeMultipleSelectHandler( "vehicleSearch", "vehicles", "addVehicle", "selectedVehiclesList", "hiddenVehiclesContainer", "vehicles[]", "vehículo", {!! json_encode(old('vehicles', $report->vehicles->pluck('id')->toArray())) !!} );
    ...
    if (mapElement && typeof L !== 'undefined') {
        let defaultLat = parseFloat("{{ $report->location->latitude ?? config('leaflet.map_center_latitude', -33.2951) }}");
        let defaultLng = parseFloat("{{ $report->location->longitude ?? config('leaflet.map_center_longitude', -66.3379) }}");
        let defaultZoom = parseInt("{{ config('leaflet.zoom_level', 14) }}");

        // Priorizar 'old' data si existe (tras validación fallida), sino usar datos del reporte
        let initialLat = parseFloat("{{ old('latitude', $report->location->latitude ?? defaultLat) }}");
        let initialLng = parseFloat("{{ old('longitude', $report->location->longitude ?? defaultLng) }}");
        let initialAddress = "{{ old('address', $report->location->address ?? '') }}";
        ...
        // No es necesario llamar a updateMapFieldsFromManual aquí si los inputs ya están poblados
        // con old() o datos del reporte. El marcador ya está en la posición correcta.
